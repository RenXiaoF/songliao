export default class websocket {
    constructor({ heartCheck, isReconnection, userInfo }) {
        // 是否连接
        this._isLogin = false;
        // 当前网络状态
        this._netWork = true;
    ...
    _start() {
        this._timeoutObj = setInterval(() => {
                let send_data = {
                    ...
                    'providerid':this.userinfo.store_id,
                    'type':'TransportationMaterials'
                };
                // websocket 发送待审核的信息。
                var login_data = {type: 'pull_message', result: send_data};
    ...
    _onSocketOpened() {
        ...
        let send_data = {
            ...
            'store_id':  _this.userinfo.store_id,
            'providerid':_this.userinfo.store_id,
            'type':'TransportationMaterials'
        };
        // websocket 发送待审核的信息。
        var login_data = {type: 'pull_message', result: send_data};
    ...
    sendWebSocketMsg(options) {
        var init8arr = new Uint8Array(options.data);
        wx.sendSocketMessage({
            data: init8arr,
            success(res) {
